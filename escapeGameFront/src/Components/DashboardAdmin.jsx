...
function Dashboard() {
    ...
    useEffect(() => { {/*Effet pour récupérer les réservations au montage du composant */}
        async function fetchReservations() {
            try {
                const response = await PageAdmin.fetchPageAdmin(); {/* Appel à la fonction du service pour récupérer les réservations */}
    ...
    return (
        <div className="w-full p-4 text-white"> {/* Conteneur principal avec padding et texte blanc */} 
            <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-center sm:text-left">Tableau de bord Admin</h1> 

            <div className="overflow-x-auto bg-white text-black rounded-xl shadow p-2 sm:p-4">  {/* Conteneur pour la table avec styles */}
                <table className="min-w-full text-sm sm:text-base"> {/* Table responsive avec taille de texte adaptative */}
                    <thead className="bg-gray-100"> {/* En-tête de la table avec fond gris clair*/}
                        <tr>
                    ...
                    <tbody>
                        {reservations.map((reservation) => ( // parcours des réservations pour créer les lignes du tableau
                            <tr key={reservation.id_booking} className="border-b hover:bg-gray-50"> {/*  identifiant unique pour chaque ligne */}
                                ...
                                <td className="hidden md:table-cell px-4 py-2 text-center">{new Date(reservation.date_booking).toLocaleDateString("fr-FR")}</td> {/* Formatage de la date en français */}
                                <td className="hidden lg:table-cell px-4 py-2 text-center">{new Date(reservation.hours_selected).toLocaleTimeString("fr-FR", { hour: '2-digit', minute: '2-digit' })}</td> {/* Formatage de l'heure en français */}
